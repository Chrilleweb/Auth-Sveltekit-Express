	...
	{/if}
      <form on:submit|preventDefault={resetPassword}>
          <div class="mb-4">
              ...
              <input type="password" id="newPassword" bind:value={newPassword} class="form-input mt-1 block w-full" required />
          </div>
          <div class="mb-4">
              <label for="confirmPassword" class="block text-sm font-medium text-gray-700">Confirm New Password:</label>
              <input type="password" id="confirmPassword" bind:value={confirmPassword} class="form-input mt-1 block w-full" required />
          </div>
          <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" disabled={processing}>
